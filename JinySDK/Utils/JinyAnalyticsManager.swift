...
class JinyAnalyticsManager {
    ...
    init(_ analyticsDelegate:JinyAnalyticsManagerDelegate) {
        ...
        let events = pref.object(forKey: "jiny_saved_events") as? Array<Dictionary<String,Any>> ?? []
        flushEvents(events)
    }
    ...
    func saveEvent(payload:Dictionary<String,Any>, isSuccess:Bool) {
        ...
        var savedEvents = prefs.object(forKey: "jiny_saved_events") as? Array<Dictionary<String,Any>> ?? []
        ...
        prefs.set(savedEvents, forKey: "jiny_saved_events")
        prefs.synchronize()
    }
    ...
    func saveEvents(payload:Array<Dictionary<String,Any>>) {
        ...
        var savedEvents = prefs.object(forKey: "jiny_saved_events") as? Array<Dictionary<String,Any>> ?? []
        savedEvents += payload
        prefs.set(payload, forKey: "jiny_saved_events")
        prefs.synchronize()
    }
