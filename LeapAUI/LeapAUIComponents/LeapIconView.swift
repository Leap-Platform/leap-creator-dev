...
/// LeapIconView which holds a webview with Leap Icon image.
class LeapIconView: UIView {
    
    /// Delegation when there is a change in icon state.
    weak var stateDelegate: LeapIconStateDelegate?
    
    /// enum property to track the state.
    var iconState: LeapIconState = .rest {
...
extension LeapIconView: WKScriptMessageHandler {
