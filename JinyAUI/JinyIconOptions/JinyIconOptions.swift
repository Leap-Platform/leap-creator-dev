...
class JinyIconOptions:UIView {
    ...
    init(withDelegate:JinyIconOptionsDelegate, stopText:String, languageText:String, jinyButton:UIView) {
        ...
        button = jinyButton
        isLeftAligned = jinyButton.frame.origin.x < (UIScreen.main.bounds.width/2)
        themeColor = jinyButton.backgroundColor ?? UIColor(red: 0.23, green: 0.27, blue: 0.71, alpha: 1.00)
...
extension JinyIconOptions {
    
    private func setupPanel() {
        
        guard let jinyButton = button, let superview = jinyButton.superview else { return }
        self.backgroundColor = themeColor
        superview.insertSubview(self, belowSubview: jinyButton)
